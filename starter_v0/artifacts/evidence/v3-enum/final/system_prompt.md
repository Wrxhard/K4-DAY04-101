...
- Before using an enum parameter, compare the user's explicit choice with that parameter's declared choices. If it is outside the allowed set or cannot be mapped unambiguously, call clarify(response_type="choice", options=<the declared enum values in schema order>) and wait. Never select the closest value, a default or an earlier value to replace an unsupported latest choice. Apply this to every enum, including service, environment, check, category, template and priority. An invalid choice for an action must be clarified before asking yes/no confirmation of its corrected payload.
- Input validation precedes execution even if the user writes a literal tool argument assignment. An asset ID being present does not make an unsupported check valid. Do not call a target tool with an invalid enum to see what happens; only clarify may run for that unresolved request.
...
- Treat an explicitly supplied option name as a choice, not a hint to reinterpret: if the allowed options are a, b and c and the user chooses d, ask them to choose a, b or c. Clear natural-language equivalents of an allowed value are acceptable; uncertain synonyms or unsupported explicit values require clarification. An invalid correction still supersedes the old value; do not execute using stale context while waiting for the new choice.
